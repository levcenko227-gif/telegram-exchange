...
app.post('/api/orders/:id/complete', isUserAuth, (req, res) => {
  ...
  db.prepare('UPDATE users SET balance_rub = balance_rub + ? WHERE id = ?').run(order.amount_rub, req.session.userId);
  db.prepare('INSERT INTO notifications (user_id, message, type) VALUES (?, ?, ?)').run(req.session.userId, `✅ Ордер ${order.order_number} выполнен. Начислено ${order.amount_rub} ₽`, 'success');
  res.json({ success: true });
});
...
app.get('/api/user/stats', isUserAuth, (req, res) => {
  ...
  const getStats = (from) => db.prepare("SELECT COALESCE(SUM(amount_usdt),0) as usdt, COALESCE(SUM(amount_rub),0) as rub, COALESCE(SUM(earned_rub),0) as earned, COUNT(*) as deals FROM deposits WHERE user_id = ? AND status = 'confirmed' AND created_at >= ?").get(userId, from);
  const getOrders = (from) => db.prepare("SELECT COUNT(*) as count FROM orders WHERE user_id = ? AND status = 'completed' AND created_at >= ?").get(userId, from).count;
  res.json({
    today: { ...getStats(today), orders: getOrders(today) },
    yesterday: { ...getStats(yesterday), orders: getOrders(yesterday) },
    week: { ...getStats(weekAgo), orders: getOrders(weekAgo) },
    lastWeek: { ...getStats(lastWeekStart), orders: getOrders(lastWeekStart) },
    month: { ...getStats(monthAgo), orders: getOrders(monthAgo) },
    total: { ...db.prepare("SELECT COALESCE(SUM(amount_usdt),0) as usdt, COALESCE(SUM(amount_rub),0) as rub, COALESCE(SUM(earned_rub),0) as earned, COUNT(*) as deals FROM deposits WHERE user_id = ? AND status = 'confirmed'").get(userId), orders: db.prepare("SELECT COUNT(*) as count FROM orders WHERE user_id = ? AND status = 'completed'").get(userId).count }
  });
});
...
app.post('/api/admin/withdrawals/:id/reject', isAdminAuth, (req, res) => {
  ...
  if (w) db.prepare('UPDATE users SET balance_rub = balance_rub + ? WHERE id = ?').run(w.amount_rub, w.user_id);
