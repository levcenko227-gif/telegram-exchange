/* ==========================================================================
   ...
   Full server API & SSE integration | Safe DOM binding
   ========================================================================== */
...
async function handleLogin() {
    ...
    if (!username || !password) { showError(errEl, 'Введите логин и пароль'); return; }
    ...
    try {
        ...
        else { showError(errEl, data.error || 'Неверный логин или пароль'); }
    } catch (e) { showError(errEl, 'Ошибка подключения к серверу'); }
}
...
function showApp() {
    ...
    if (needsPasswordChange || needs2FA || needsVerification) showSetupWizard();
    else showDashboard();
    ...
    try {
        const res = await fetch('/api/verification/submit', {
            ...
            body: JSON.stringify({
                ...
                phone, telegram_link: telegram, social_links: social
            })
        });
        const data = await res.json();
        if (data.success) {
...
function showPage(page) {
    ...
    const titles = {
        home: 'Главная', orders: 'Ордера', appeals: 'Апелляции', balance: 'Баланс',
        withdraw: 'Вывод', requisites: 'Реквизиты', notifications: 'Уведомления', profile: 'Профиль'
    };
...
function updateUserUI() {
    ...
    if (currentUser.totp_enabled) {
        s.textContent = 'Подключена ✓'; s.classList.add('active');
        b.textContent = 'Отключить'; b.onclick = () => disable2FA();
    } else {
        s.textContent = 'Не подключена'; s.classList.remove('active');
        b.textContent = 'Настроить'; b.onclick = () => setup2FA();
    }
    if (window.lucide) lucide.createIcons();
}
...
async function loadRate() {
    try {
        ...
        updateCoinSuffix();
    } catch (e) { console.error('Rate load error', e); }
}
...
function updateCoinSuffix() {
    ...
    const el = document.getElementById('coin-suffix');
...
function animateElementCount(id, targetVal) {
    ...
    function step(now) {
        ...
        el.textContent = formatRub((targetVal || 0) * ease);
...
function renderOrder(o) {
    ...
    const reqHtml = o.requisite ? `
        <div style="margin-top:8px;padding:10px;background:var(--bg-2);border:1px solid var(--brd-1);border-radius:var(--r-md);font-size:13px;">
    ...
    return `
        <div style="padding:16px;background:var(--bg-2);border:1px solid var(--brd-1);border-radius:var(--r-md);margin-bottom:12px;">
            ...
            <div style="font-size:1.3rem;font-weight:800;font-family:var(--font-mono);margin-top:6px;color:var(--mint);">${formatRub(o.amount_rub)}</div>
            ${reqHtml}${timerHtml}${actionsHtml}
            <div style="font-size:12px;color:var(--txt-3);margin-top:8px;">${formatDate(o.created_at)}</div>
        </div>`;
}
...
async function completeOrder(id) {
    ...
    const data = await res.json();
    if (data.success) { showToast('Ордер выполнен!', 'success'); loadOrders(); loadStats(); }
    else showToast(data.error, 'error');
}

async function failOrder(id) {
    if (!confirm('Отметить ордер как неуспешный?')) return;
    const res = await fetch(`/api/orders/${id}/fail`, { method: 'POST' });
    const data = await res.json();
    if (data.success) { showToast('Ордер отмечен как неуспешный', 'error'); loadOrders(); }
    else showToast(data.error, 'error');
}
...
function renderAppeal(a) {
    ...
    const receiptHtml = a.receipt_url ? `
        <div style="margin-top:8px;"><img src="${a.receipt_url}" style="max-width:100%;max-height:200px;border-radius:var(--r-md);cursor:pointer;border:1px solid var(--brd-2);" onclick="window.open(this.src)" alt="Чек"></div>` : '';

    let actionHtml = '';
    if (a.status === 'pending' && !a.client_action) {
        actionHtml = `
            <div style="margin-top:12px;">
                <div class="form-group"><label>Выписка из банка (обязательно)</label><input type="file" id="appeal-statement-${a.id}" accept="image/*"></div>
    ...
    return `
        <div style="padding:16px;background:var(--bg-2);border:1px solid var(--brd-1);border-radius:var(--r-md);margin-bottom:12px;">
            <div style="display:flex;justify-content:space-between;align-items:center;">
                <span style="font-weight:700;font-family:var(--font-mono);">${a.appeal_number}</span>
                <span class="order-status status-${a.status}">${appealStText(a.status)}</span>
            </div>
            <div style="font-size:1.3rem;font-weight:800;font-family:var(--font-mono);margin-top:6px;color:var(--rose);">${formatRub(a.amount_rub)}</div>
            ${a.order_number ? `<div style="font-size:13px;color:var(--txt-3);margin-top:4px;">Ордер: ${a.order_number}</div>` : ''}
            ${reqHtml}${receiptHtml}
            ${a.description ? `<div style="font-size:13px;color:var(--txt-2);margin-top:8px;">${a.description}</div>` : ''}
            ${actionHtml}
            <div style="font-size:12px;color:var(--txt-3);margin-top:8px;">${formatDate(a.created_at)}</div>
        </div>`;
}
...
async function acceptAppeal(id) {
    ...
    const data = await res.json();
    if (data.success) { showToast('Выписка отправлена', 'success'); loadAppeals(); }
    else showToast(data.error, 'error');
}
...
async function confirmDeposit() {
    ...
    try {
        ...
        const data = await res.json();
        if (data.success) { showToast('Заявка отправлена!', 'success'); cancelDeposit(); loadDeposits(); }
        else showToast(data.error, 'error');
...
async function loadDeposits() {
    try {
        ...
        container.innerHTML = list.map(d => `
            <div style="padding:14px;background:var(--bg-2);border:1px solid var(--brd-1);border-radius:var(--r-md);margin-bottom:10px;">
                <div style="display:flex;justify-content:space-between;align-items:center;">
...
async function createWithdrawal() {
    ...
    try {
        ...
        if (data.success) {
            ...
            loadStats(); refreshUserProfile(); loadWithdrawals();
        } else showToast(data.error, 'error');
    } catch (e) { showToast('Ошибка', 'error'); }
}
...
async function loadRequisites() {
    try {
        ...
        if (container) {
            if (!list.length) container.innerHTML = '<p style="color:var(--txt-3);font-size:14px;padding:12px;">Нет реквизитов</p>';
            else {
                container.innerHTML = list.map(r => `
                    <div style="padding:14px;background:${r.is_active ? 'var(--mint-subtle)' : 'var(--bg-2)'};border-radius:var(--r-md);margin:8px 0;display:flex;justify-content:space-between;align-items:center;border:1px solid ${r.is_active ? 'rgba(52,211,153,0.3)' : 'var(--brd-1)'};flex-wrap:wrap;gap:8px;">
                        <div>
                            <div style="font-weight:700;color:var(--txt-1);">${r.bank}</div>
                            <div style="font-size:13px;color:var(--txt-2);">${r.name} | ${r.phone}</div>
                            ${r.is_active ? '<span style="font-size:11px;color:var(--mint);font-weight:700;">🟢 Активен</span>' : '<span style="font-size:11px;color:var(--txt-3);">Выключен</span>'}
                        </div>
                        <div style="display:flex;gap:8px;">
                            ...
                            <button class="btn btn-small btn-secondary" onclick="archiveReq(${r.id})">📦</button>
                        </div>
                    </div>`).join('');
            }
        }
    ...
    } catch (e) { console.error('Requisites error', e); }
}
...
async function loadArchivedRequisites() {
    try {
        ...
        if (!container) return;
        if (!list.length) { container.innerHTML = '<p style="color:var(--txt-3);font-size:13px;padding:8px;">Архив пуст</p>'; return; }
        container.innerHTML = list.map(r => `
            <div style="padding:10px;background:var(--bg-2);border-radius:var(--r-md);margin:6px 0;font-size:13px;display:flex;justify-content:space-between;align-items:center;border:1px solid var(--brd-1);gap:8px;">
...
async function deactivateReq(id) { await fetch(`/api/requisites/${id}/deactivate`, { method: 'POST' }); showToast('Деактивирован', 'info'); loadRequisites(); }
...
async function disable2FA() {
    ...
    const data = await res.json();
    if (data.success) { showToast('2FA отключена', 'info'); currentUser.totp_enabled = false; updateUserUI(); }
    else showToast(data.error, 'error');
}
...
function connectSSE() {
    ...
    eventSource.addEventListener('deposit_rejected', () => { showToast('❌ Депозит отклонён', 'error'); loadDeposits(); });
    eventSource.addEventListener('withdrawal_confirmed', () => { showToast('✅ Вывод подтверждён!', 'success'); loadWithdrawals(); });
    eventSource.addEventListener('withdrawal_rejected', () => { showToast('❌ Вывод отклонён', 'error'); loadWithdrawals(); });
    ...
    eventSource.addEventListener('order_completed', () => { loadOrders(); loadStats(); });
    eventSource.addEventListener('order_failed', () => { loadOrders(); });
